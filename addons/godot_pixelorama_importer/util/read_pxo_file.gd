...
static func read_pxo_zip_file(source_file: String, image_save_path: String) -> Result:
    ...
    for frame_index in range(frame_count):
        ...
        for cel_index in range(frame.cels.size()):
            ...
            var layer_image := Image.create_from_data(size.x, size.y, false, Image.FORMAT_RGBA8, layer_raw_data);
        ...
        if frame_image:
            ...
            spritesheet.blit_rect(frame_image, Rect2(Vector2.ZERO, size), Vector2(size.x * frame_index, 0));
